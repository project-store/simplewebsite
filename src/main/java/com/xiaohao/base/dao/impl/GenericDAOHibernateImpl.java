package com.xiaohao.base.dao.impl;
...
public class GenericDAOHibernateImpl<T extends Serializable, PK extends Serializable>
	...
	{
		String queryString = "from " + this.entityClass.getName() + " where 1=1 and " + condition;
	...
	{
		String queryString = "from " + this.entityClass.getName() + " where 1=1 and " + fKName + "=" + fKId;
		this.getHibernateTemplate().setMaxResults(max);
        return (List<T>)this.getHibernateTemplate().find(queryString);
	}
	...
	{
		String queryString = "from " + this.entityClass.getName() + " where 1=1 and " + fKName + "=" + fKId + " order by createTime desc";
		this.getHibernateTemplate().setMaxResults(max);
        return (List<T>)this.getHibernateTemplate().find(queryString);
	}
	...
	public List<T> findBySingleProperty(String propertyName, String propertyValue) 
	{
		String queryString = "from " + this.entityClass.getName() + " where 1=1 and " + propertyName + "=" + propertyValue;
        return (List<T>)find(queryString);
	}
	
	public List<T> findBySingleProperty(int max, String propertyName, String propertyValue) 
	{
		String queryString = "from " + this.entityClass.getName() + " where 1=1 and " + propertyName + "=" + "'" + propertyValue + "'";
		this.getHibernateTemplate().setMaxResults(max);
        return (List<T>)this.getHibernateTemplate().find(queryString);
	...
	{
		return (T)getHibernateTemplate().findByNamedParam(queryString, paramName, value).get(0);
	}
	// 使用带命名的参数的HSQL语句检索数据
	...
	public Page findPageByCriteria(final DetachedCriteria detachedCriteria,
		...
		return (Page) getHibernateTemplate().execute(new HibernateCallback() {
			public Object doInHibernate(Session session)
				...
				int firstResult = (startIndex - 1) * pageSize<0?0:(startIndex - 1) * pageSize;
	...
	public Page queryForpage(String select, Object[] values, int pagestart,
			int pagesize, LinkedHashMap<String, String> orderby) {
		Page page = new Page(((SystemContext.getOffset()-SystemContext.getOffset()%SystemContext.getPagesize())/SystemContext.getPagesize())+1,SystemContext.getPagesize());
	...
	/**
	 * 通过sql查询分页列表
	 * @param listSql 获取列表的SQL 
	 * @param countSql 查询总数的SQL
	 * @param rowMapper 结果数组转化成对象 
	 * @param pagestart 
	 * @param pagesize
	 * @param orderby
	 * @return
	...
	public Page sqlQueryForPage(String sql,Object[] params,int offSet,int pagesize,LinkedHashMap<String, String> orderby){
		...
		Page page = new Page((offSet-offSet%pagesize)/pagesize+1,pagesize);
...
//	//通过sql获取对象
//	public List<T> sqlQueryForList(String sql,Object[] params,RowMapper<T> rowMapper){
...
//	}
	...
	//通过sql获取对象
	/*
	public List<T> sqlQueryForList(String sql,Object[] params,RowMapper<T> rowMapper,Integer offSet,Integer pagesize){
		
		List<Map<String,Object>> list = sqlQueryForList(sql, params, offSet, pagesize);
		
		//结果数据转化成Domain Object
		List<T> items = new ArrayList<T>();
		for(Map<String,Object> item:list){
			items.add(rowMapper.mapRow(item));
		}
	...
	protected String getSqlPagingQuery(String sql){
		StringBuffer buffer = new StringBuffer("select * from (select t1.*,rownum rownum_ from (")
			.append(sql)
	...
	public Page sqlQueryForPage(String sql,Object[] params,LinkedHashMap<String, String> orderby){
		return sqlQueryForPage(sql,params,SystemContext.getOffset(),SystemContext.getPagesize(),orderby);
	}
	 ...
	 * @param params 参数数组
	 * @param orderby 排序字符串
	 * @param flag true:直接查询sql的总记录；false:截取sql后，再查询总记录
	 * 
	 * @return 分页列表
	 */
	public Page sqlQueryForPage(String sql,Object[] params,LinkedHashMap<String, String> orderby , boolean flag){
			int offSet = SystemContext.getOffset();
			int pagesize = SystemContext.getPagesize();
			//查询总数
			SQLQuery countQuery = getSession().createSQLQuery(flag?getCountSQL(sql):getCountQuery(sql));
			for(int i=0;i<params.length;i++){
				countQuery.setParameter(i, params[i]);
			}
